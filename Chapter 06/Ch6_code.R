...
# Parametric model
regfits =survreg(Surv(survtime, censor) ~  Group,  data=data.frame(Dsurv), 
                 dist = 'lognormal')

# Parametric model with additional covariates
regcovfits = survreg(Surv(survtime, censor) ~  Group + Age + Gender + Diag + 
                      Family_hist,  data = data.frame(Dsurv), 
                     dist = 'lognormal')
ggsurvreg <- ggsurvplot(predict(regfits), data = Dsurv, conf.int = TRUE,  
                        ggtheme = theme_light(), pval = F, 
                        conf.int.style = "step", 
                        legend.lab = c('Medical-therapy group', 'Surgery group'), 
                        palette = gray(seq(0.5,0, len = 2)), 
                        xlab = 'Time (days)')
print(ggsurvreg)
...
plot(wt1[1, ] ~ t, type = 'l', lwd = 3, lty = 1, ylim = c(0, 4), xlab = 'time', 
     ylab = 'Hazard function')

lines(wt2[1, ]~t, type = 'l', lwd = 3, lty = 2)
lines(expn[1, ]~t, type = 'l', lwd = 3, lty = 3)
lines(logres[1, ]~t, type = 'l', lwd = 3, lty = 4)
legend('topleft',c('Increasing Weibull', 'Decreasing Weibull', 'Exponential', 
                   'Log-normal'), lty = 1:4, )

plot(wt1[2, ] ~ t, type = 'l', lwd = 3, lty = 1, ylim = c(0, 1.2), xlab = 'time', 
